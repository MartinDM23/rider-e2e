...
test('Registro Inversionista con 2captcha', async ({ page }) => {
  ...
  const url = 'https://rider-litigation-finance-web-stating-dot-total-pad-308418.uc.r.appspot.com/preregistro-inversionista-paso0';

  await page.goto(url);

  // Pagina de Preregistro
  
  await page.locator('input#preregistered_user-email').fill('rider.tester23+e2einversionista@gmail.com');
  ...
  await page.locator('input[id="user_first_name"]').fill('Test Invest e2e');

  await page.waitForTimeout(3_000);

  await page.locator('input[id="user_last_name"]').fill('Martin e2e');
  //await page.pause();
  ...
  await page.locator('ion-icon[class="icoGrande md hydrated"]').selectOption('Entity') 
